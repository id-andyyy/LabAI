...
Парси весь документ целиком (не только задание, но и теорию, методические указания, примеры — всё, что есть в файле). Цель — сохранить полный контекст исходного документа в удобном для агента формате: текст + ссылки на извлечённые изображения.

- Если есть venv и файл PDF/DOCX — запусти полный парсинг:
  ```
  .claude/lab/venv/bin/python .claude/lab/scripts/parse_assignment.py --full --images-dir .claude/lab/source-images <файл>
  ```
  Скрипт извлечёт весь текст и все изображения из документа. Изображения сохранятся в `.claude/lab/source-images/`, а текст будет содержать Markdown-ссылки на них.
...
- Если Python не установлен, а файл PDF/DOCX — попроси пользователя скопировать весь текст документа в чат или в файл task.txt.

Сохрани полный результат парсинга в `.claude/lab/source-material.md`.
