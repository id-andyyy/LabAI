...
Пользователь хочет настроить проект под себя. Нужно узнать о его устройствах и сети, собрать данные о сетевых интерфейсах, уточнить пожелания к оформлению, а затем записать всё это в CLAUDE.md. Если какие-то данные уже заполнены - пропускай их и переходи к следующему вопросу.
...
1. **Подписи рисунков и таблиц.** Как у тебя принято подписывать рисунки и таблицы? Введи пример подписи рисунка и пример подписи таблицы (например: «Рисунок 1. Результат команды» или «Рисунок 1 — Результат команды»).
...
3. **Лицо и стиль отчёта.** От чьего имени вести отчёт и в каком лице? Введи ФИО автора (если нужно) и укажи лицо: первое («я сделал») или безличное («было выполнено»).
4. **Формат отчёта.** В каком формате нужен готовый текст отчёта: plain text (чистый текст без разметки) или Markdown?
5. **Предмет.** Укажи предмет, для которого выполняется лабораторная работа (например: «Сетевые технологии», «Клиент-серверные приложения», «Операционные системы и среды», «Информационная безопасность»).
6. **ФИО и группа студента**. Введи своё ФИО и группу (например, «Иванов Иван Иванович, БИВТ-24-2»). Если не хочешь указывать — выбери «Не указывать».
7. **Дополнительное исследование**. Предусмотрено ли в отчёте дополнительное исследование (Да/Нет)?
...
Покажи пользователю настройки, которые все еще не заданы. Если все задано - просто сообщи об успехе. Сообщи, что настройка завершена и теперь все команды (/lab-instructions, /lab-report и т.д.) будут учитывать его окружение.
